...
import { EsTsExampleTextFieldView } from "./EsTsExampleTextField.view.tsx";
...
const counterRowView = (row: CounterRow) => (
  ...
  >
    ...
    <span className="ml-auto flex gap-2">
      {Arr.map(row.actions, (action) =>
        EsTsExamplePostFormView(action.form, EsTsExampleSubmitButtonView(action.button)),
      )}
    </span>
  </li>
);
    ...
    <main className="mx-auto grid w-full max-w-2xl gap-8 p-6" data-component="CounterPage">
      ...
      {EsTsExamplePostFormView(
        ...
        <section className="flex flex-wrap items-end gap-4 rounded-box bg-base-100 p-4">
          {EsTsExampleTextFieldView(model.newCounterField)}
          {EsTsExampleSubmitButtonView(model.createButton)}
        </section>,
      )}

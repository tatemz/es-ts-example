...
export const viewRole = {
  matches: isViewFilename,
  messages: {
    viewInvalidModel:
      "View files must accept their owned model type imported from `../../models/<layer>/<stem>.model.ts`.",
    viewInvalidName: "This view file must export `{{viewName}}` for `{{modelName}}`.",
    viewInvalidParameter: "Views should receive `model: {{modelName}}`.",
    viewInvalidViewType:
      "Views must be typed as `View<{{modelName}}>` or `View<{{modelName}}, HtmlSlotContent>`.",
    viewNotExported: "View files must export their owned renderable view.",
    viewOneView:
  ...
  check(context, node, filename) {
    Fn.pipe(
      ...
      Arr.map((typeExport) =>
        context.report({ node: typeExport.node, messageId: "viewTypeExport" }),
      ),
    );
    ...
    if (view === undefined) {
      context.report({ node, messageId: "viewOneView", data: { ownedViewName } });
      return;
    }
    ...
    if (!validModelParameter(details)) {
      context.report({
        ...
        messageId: "viewInvalidParameter",
    ...
    if (!validModelImport(details)) {
      context.report({ node: details.first, messageId: "viewInvalidModel" });
      return;
    }
    ...
    if (!Fn.pipe(exportedNames(node), Arr.contains(view.name))) {
      context.report({ node: view.node, messageId: "viewNotExported" });
    }
  },
};

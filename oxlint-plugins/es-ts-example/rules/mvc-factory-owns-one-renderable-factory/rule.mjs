...
import { modelImportSourceForStem } from "../shared/paths.mjs";

export const mvcFactoryOwnsOneRenderableFactoryRuleName = "mvc-factory-owns-one-renderable-factory";
...
const importsOwnedModel = (program, modelName, stem) =>
  Fn.pipe(
    program.body,
    Arr.some((statement) => {
      if (
        statement.type !== "ImportDeclaration" ||
        statement.source.value !== modelImportSourceForStem(stem)
      ) {
        return false;
      }
...
export const mvcFactoryOwnsOneRenderableFactory = {
  meta: {
    type: "problem",
    docs: {
      description:
        "Require each web MVC factory file to own exactly one model factory and export only model constructors.",
    },
    messages: {
      invalidExport:
      ...
      missingFactory: "Factory files must export `{{factoryName}}`.",
      missingImport:
        "Factory files must import the owned model schema from `../models/{{stem}}.model.ts`.",
      missingConstructor:
  ...
  create(context) {
    ...
    return {
      Program(node) {
        ...
        if (!importsOwnedModel(node, ownership.modelName, ownership.stem)) {
          context.report({
            node,
            messageId: "missingImport",
            data: { stem: ownership.stem },
          });
        }
        ...
        Fn.pipe(
          bindings,
          Arr.map((binding) => {
            if (binding.kind === "default" || binding.kind === "re-export-all") {
              context.report({
                node: binding.node,
            ...
            if (binding.kind === "type-export" || binding.kind === "re-export") {
              context.report({
                node: binding.node,
                messageId: binding.kind === "type-export" ? "typeExport" : "invalidExport",
                data: { modelName: ownership.modelName },
              });
              return;
            }
            ...
            const returnTypeName = explicitReturnTypeName(binding.declaration, binding.variable);
            ...
            if (returnTypeName !== ownership.modelName) {
              context.report({
                node: binding.node,
                messageId: "invalidReturnType",
                data: { modelName: ownership.modelName },
